...
let rec assoc (d,k,l) = 
	match l with
	| []-> d (*if no more ki and not found*)
	| (s, i)::t-> 
		if s=k then i  (*first string matches k*)
		else assoc (d, k, t) (*recurse using tail of l*)
...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
      ...
      | h::t -> 
		...
		| false-> List.rev(helper(seen@[h], t))
		| true-> List.rev(helper(seen, t))
...
let rec wwhile (f,b) =
	match f b with
	| (b',c')-> 
		if c' =false then b' (*last case, f returned a c' that is false*)
		else wwhile (f, b');;(*keep calling f on b*)


let _= let f x = let xx = x*x*x in (xx, xx < 100) in
wwhile (f, 2);;
...
let fixpoint (f,b) = wwhile ((fun b-> (f b, if f b=b then false else true)),b);;
...
let rec exprToString e = 
	...
	| Cosine y-> Printf.sprintf "cos(pi*%s)" (exprToString y)
	| Average (z,a)-> Printf.sprintf "((%s+%s)/2)" (exprToString z) (exprToString a) 
	| Times (b,c)-> Printf.sprintf "%s*%s" (exprToString b) (exprToString c)
	| Custom1 (a,b)-> Printf.sprintf "(%s*(%s-1))/3" (exprToString a) (exprToString b)
	| Custom2 (a,b,c)-> Printf.sprintf " (%s-%s)/(%s+pi)" (exprToString a)(exprToString b)(exprToString c) 
	| Thresh (d,e,f,g)-> (Printf.sprintf "(%s<%s?%s:%s)" (exprToString d) (exprToString e) (exprToString f) (exprToString g));;
...
let buildCustom1(e1,e2)            = Custom1(e1,e2)
let buildCustom2(e1,e2,e3)         = Custom2(e1,e2,e3)
...
let rec eval (e,x,y) = 
	...
	| VarX -> x
	| VarY -> y
	| Sine z-> sin(pi*.eval (z, x, y))
	| Cosine z-> cos(pi*.eval (z, x, y))
	| Average (a,b)-> ((eval(a,x,y))+.(eval(b,x,y)))/.2.0
	| Times (a,b)-> (eval(a,x,y))*.(eval(b,x,y)) 
	| Custom1(a,b)-> ((eval(a,x,y))*.(eval(b,x,y)-.1.0))/.3.0
	| Custom2(a,b,c)->  ((eval(a,x,y))-.(eval(b,x,y)))/.((eval(c,x,y))+.pi)
	| Thresh (a,b,c,d)-> 
...
let rec build (rand, depth) = 
	match depth with
		| 0->  
			(let n=rand(0,2) in match n with
				| 0-> buildX()
				| _-> buildY())
		| _->
			...
			| _->
			(let q= rand(0,6) in match q with
			| 0-> buildTimes(build(rand, depth-1), build(rand,depth-1)) 
			| 1->buildX()
			| 2->buildY()
			| 3-> buildCustom1(build(rand, depth-1), build(rand, depth-1))
			| 4-> buildCustom2(build(rand, depth-1), build(rand, depth-1), build(rand,depth-1))
