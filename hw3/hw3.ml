  ...
  let add (l1, l2) = 
	...
	| (y,z) -> 
		match a with
			| (carry, sum)->
				match sum with    
					|[]-> if(y+z+carry)<10 then (0, [carry]@[y+z+carry])
						else (carry+1,[carry+1]@[(carry+y+z)-10]) 
					|h::t->if (y+z+h)<10 then (0, [0]@[y+z+h]@t)
						else (carry+1, [(y+z+h)/10]@[(y+z+h)-10]@t)
  in
    let base = (0,[]) in
...
let bigMul l1 l2 = 
  let f a x = 
	match a with
	| (carry,sum) ->
		(carry+1, bigAdd sum ((mulByDigit x l1) @ (clone 0 carry)))
 in
  let base = (0,[]) in
...
let runAllTests () =
  ...
  let report = 
    [runTest (sqsum, [], 0, 1, "sqsum 1");
     ...
     runTest(wrap_curried_2 bigMul,  ([8;2;6;7;4],[1;3;5;9;7]), [1;1;2;4;1;1;8;3;7;8],1,"bigMul 2");
     ] in
